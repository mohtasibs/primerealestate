...
@foreach ($blogs as $item)
    <div class="col-xl-4 col-lg-4 col-md-6">
        <div class="card rounded-4 shadow-sm h-100">
            <a href="{{ route('blog-detail', ['title' => Str::slug($item['title'])]) }}" class="d-block bg-gradient rounded-top">
                <img class="card-img-top hover-fade-out" src="{{ asset($item['img']) }}" alt="blog image">
            </a>
            <div class="card-body">
                <a href="{{ route('blog-detail', ['title' => Str::slug($item['title'])]) }}"><h4 class="fw-semibold fs-5 lh-base mb-3">{{ $item['title'] }}</h4></a>
                <p>{{ $item['desc'] }}</p>
            ...
            <div class="card-footer bg-white d-flex justify-content-between align-items-center py-3">
                <div class="text-dark fw-medium text-md"><i class="bi bi-calendar-check me-2"></i>{{ $item['date'] }}</div>
                <div class="text-muted fw-medium text-md"><i class="bi bi-eye-fill me-2"></i>{{ $item['views'] }}</div>
            </div>
        </div>
    </div>
@endforeach
